...
// --- Sheet-check / on-demand update logic ---
async function loadSiteConfig() {
  try {
    const cfg = await fetchJson("site_config.json");
    return cfg || {};
  } catch (err) {
...
async function fetchSheetCnpjs(publishedCsvUrl, options = {}) {
  if (!publishedCsvUrl) {
    throw new Error("Site not configured with published sheet URL.");
  }
  const resp = await fetch(publishedCsvUrl, { cache: "no-store" });
  if (!resp.ok) throw new Error(`Falha ao buscar planilha: ${resp.status}`);
  const text = await resp.text();
  // guard: if the response looks like HTML (login page, error page), surface a clearer error
  const st = (text || "").trim();
  if (st.startsWith("<") && st.length < 200000) {
    // likely an HTML page (Google sign-in / error) instead of CSV
    throw new Error("Resposta inesperada ao buscar a planilha (provavelmente não está pública). Verifique o link e permissões.");
  }
  const rows = parseCsv(text);
  if (!rows.length) return [];
  const header = rows[0].map((h) => (h || "").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, ""));
  // try to detect a 'cnpj' column in the header
  let cnpjIdx = header.findIndex((h) => h && h.indexOf("cnpj") !== -1);
  if (cnpjIdx === -1) {
    // fallback: try common Portuguese headers
    cnpjIdx = header.findIndex((h) => h && (h.indexOf("documento") !== -1 || h.indexOf("identificacao") !== -1));
  }
  // if a column index was explicitly provided (from site_config), use it (overrides header detection)
  if (options.columnIndex !== undefined && options.columnIndex !== null) {
    cnpjIdx = Number(options.columnIndex);
  }
  ...
  for (let i = 1; i < rows.length; i++) {
    const row = rows[i] || [];
    let raw = "";
    if (typeof cnpjIdx === "number" && cnpjIdx >= 0) {
      // only consider the configured/detected column — do NOT scan the entire row
      raw = (row[cnpjIdx] || "").toString();
    } else {
      // legacy fallback: scan the row for the first CNPJ-like value
      raw = (row.find((c) => c && c.toString().match(cnpjRegex)) || "").toString();
    }
...
async function tryFetchFundJson(cnpj) {
  try {
    const res = await fetch(`data/funds/${cnpj}.json`, { cache: "no-store" });
...
async function onRefreshFromSheetClicked() {
  const statusEl = document.getElementById("sheet-check-status");
  try {
    statusEl.textContent = "Verificando planilha...";
    const cfg = await loadSiteConfig();
    const url = cfg.sheets_published_csv_url || "";
    if (!url) {
      statusEl.textContent = "site_config.json não configurado com 'sheets_published_csv_url'.";
      return;
    }
    const sheetCnpjs = await fetchSheetCnpjs(url);
    const siteCnpjs = (state.funds || []).map((f) => normalizeCnpj(f.cnpj)).filter(Boolean);

    const added = sheetCnpjs.filter((c) => !siteCnpjs.includes(c));
    const removed = siteCnpjs.filter((c) => !sheetCnpjs.includes(c));

    const available = [];
    const missing = [];
    // for every cnpj in sheet, try to load local JSON (use existing downloaded data)
    for (const c of sheetCnpjs) {
      const data = await tryFetchFundJson(c);
      if (data) {
        available.push({ cnpj: c, nome: data.metadata?.nome || c });
      } else {
        missing.push(c);
      }
    }

    // build new state.funds list preserving sheet order, preferring available funds
    state.funds = available.map((f) => ({ cnpj: f.cnpj, nome: f.nome }));
    // append missing ones as placeholders so user sees they exist on the sheet but lack local data
    missing.forEach((c) => state.funds.push({ cnpj: c, nome: `${c} (sem dados locais)` }));
    state.filteredFunds = state.funds.slice();
    renderFundList();

    // craft status message with details and a small report area
    const parts = [];
    if (added.length) parts.push(`${added.length} novo(s)`);
    if (removed.length) parts.push(`${removed.length} removido(s)`);
    if (!parts.length) parts.push("Nenhuma alteração detectada");
    const now = new Date().toLocaleString();
    // basic summary
    statusEl.textContent = `${parts.join(" · ")} — verificado em ${now}`;

    // append a details block showing lists so user can debug immediately
    const details = document.createElement("div");
    details.className = "sheet-check-hint";
    const lines = [];
    lines.push(`<strong>Total na planilha:</strong> ${sheetCnpjs.length}`);
    lines.push(`<strong>Total no site:</strong> ${siteCnpjs.length}`);
    if (added.length) lines.push(`<strong>Novos (planilha → site):</strong> ${added.join(", ")}`);
    if (removed.length) lines.push(`<strong>Removidos (site → planilha):</strong> ${removed.join(", ")}`);
    if (missing.length) lines.push(`<strong>Sem dados locais:</strong> ${missing.join(", ")}`);
    ...
    console.error(err);
    const statusEl = document.getElementById("sheet-check-status");
    statusEl.textContent = `Erro ao verificar a planilha: ${err.message || err}`;
  }
}

function setupSheetRefreshButton() {
  const btn = document.getElementById("refresh-from-sheet");
  if (!btn) return;
  btn.addEventListener("click", onRefreshFromSheetClicked);
}

// wire button after bootstrap
document.addEventListener("DOMContentLoaded", () => {
  setupSheetRefreshButton();
});
